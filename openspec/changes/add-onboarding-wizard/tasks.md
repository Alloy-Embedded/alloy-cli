...
- [ ] 1.1 Add `OnboardingCancelledError` to `src/alloy_cli/core/errors.py` (subclass of `AlloyCliError`, `error_type="onboarding-cancelled"`); export from `__all__`; extend the uniqueness regression test.
- [ ] 1.2 Add the `onboarding-cancelled` anchor to `docs/ERROR_COOKBOOK.md` so `scripts/check_error_cookbook.py` stays green.
